...
SIG_RE = re.compile(
    r"信号(?:(?:抄|超)?收)?(?:主控|主台)?\s*([0-9零一二两三四五六七八九洞幺十百千]{1,4})"
...
def extract_report_fields(text):
    ...
    # 实测补充（22:48）：ASR 把 "QTH" 听成"收体/受体"（21:02:21"收体是咸阳"）、
    # "Q T" 后跟"是"（20:47:18"Q T是"）、"QTH是"被听成"BH是"（21:01:22
    # "BH是咸阳市财富中心"）——一并归一匹配。BH 变体必须带连接词
    # （是/在/位于），防与呼号直读"BH3XX/BH9FHT"误撞（BH+数字不匹配）。
    # 值边界：QTH 值到句读/连接词即止，避免贪心吞掉后续"天线/设备"子句
    # （实测 21:02:21 "收体是咸阳…天线一米二…"被整段吞为 QTH 值）。
    m = re.search(r"(?:QTH|Q\s*T(?:\s*H)?|收体|受体|位置|地址|所在地|"
                  r"BH(?:[是在位于]|的|是))(?:[是在位于]|的|是)?"
                  r"\s*([^，。；,;.!！?？\s]{2,24}?)(?=\s*(?:天线|设备|使用|用的是|功率|信号|，|。|$))",
                  text, re.I)
    if m and m.group(1) and not re.search(r"[A-Za-z]\d[A-Za-z]{1,3}", m.group(1)):
    ...
    # 实测补充（22:48）：ASR 把"华虹"天线品牌音变听成"滑红/华红/黄红"（20:47:18
    # "一米二的滑红棒子"、21:01:22"一米二的华红棒子"）→ 匹配前归一；"外置了一根
    # 一米二的滑红棒子"无"天线"字样 → 用"外置/外接/加装/装了一根"引导句式兜底；
    # "天馈使用的是一米二的华红棒子"（21:01:22，天馈=天线+馈线习惯语）→ 同配。
    ant_text = text
    m = re.search(r"(?:天线|天馈)(?:是|为|的|用的)?\s*([\u4e00-\u9fffA-Za-z0-9\-]{2,12})",
                  text, re.I)
    if not m:
        m = re.search(r"(?:外置|外接|加装|装了|装了一根|装个)(?:了|的|一根|个)?"
                      r"\s*([\u4e00-\u9fffA-Za-z0-9\-]{2,12})", text, re.I)
    if m:
        ant = re.sub(r"^(?:使用的是|用的|使用)", "", m.group(1).strip())
        ant = re.sub(r"滑红|华红|黄红", "华虹", ant)   # ASR 音变归一（华虹天线品牌）
    ...
    # 实测补充（22:48）：友台常读"25个瓦特"/"45个瓦特"（"个"与"瓦特"连读），
    # 正则须覆盖 瓦特/个瓦特/瓦/W 四种单位写法。
    m = re.search(r"(\d+(?:\.\d+)?)\s*(?:个)?\s*(?:瓦特|瓦|W)", text, re.I) or \
        re.search(r"([零一二两三四五六七八九洞幺])\s*(?:个)?\s*(?:瓦特|瓦)", text)
    if m:
        p = m.group(1)
